...
print("Maximum Degree is ", maxDegree(G))
...
print("Minimum Degree is " ,MinimumDegree(G)[0])
...
def maximal_independent_set(G2, nodes=None):
 
    if not nodes:
        nodes = set([random.choice(G2.nodes())])     # pick a random node
    else:
        nodes = set(nodes)
    if not nodes.issubset(G2):
        raise nx.NetworkXUnfeasible("%s is not a subset of the nodes of G" % nodes)
 
    # All neighbors of nodes
    neighbors = set.union(*[set(G2.neighbors(v)) for v in nodes])
    if set.intersection(neighbors, nodes):
        raise nx.NetworkXUnfeasible("%s is not an independent set of G" % nodes)
 
    indep_nodes = list(nodes)       # initial
    available_nodes = set(G2.nodes()).difference(neighbors.union(nodes)) # available_nodes = all nodes - (nodes + nodes' neighbors)
    ...
    while available_nodes:
        ...
        available_nodes.difference_update(G2.neighbors(node) + [node])   # available_nodes = available_nodes - (node + node's neighbors)
 
    return indep_nodes
maxIndset=maximal_independent_set(G2)
print("MAX SET",maxIndset)
